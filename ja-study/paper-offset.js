(function (paper) {
  ...
  paper =
    paper && Object.prototype.hasOwnProperty.call(paper, "default")
      ? paper["default"]
      : paper;
    ...
    if (origin.isSmooth()) {
      ...
      if (intersection.length === 0) {
        if (distance > Math.abs(offset) * 0.1) {
          ...
          switch (joinType) {
            case "miter":
              var join = getPointLineIntersections(
                curve1.point2,
  ...
  function isSameDirection(partialPath, fullPath) {
    ...
    var originOffset1 = fullPath.getNearestLocation(
      partialPath.getPointAt(sampleOffset),
    ).offset;
    var originOffset2 = fullPath.getNearestLocation(
  ...
  function offsetSimpleShape(path, offset, join, limit) {
    ...
    var newPath = removeIntersection(
      new paper.Path({
        segments: segments,
        insert: false,
        closed: path.closed,
    ...
    if (
      source.closed &&
      ((source.clockwise && offset < 0) || (!source.clockwise && offset > 0))
    ) {
  ...
  function connectSide(outer, inner, offset, cap) {
    ...
    switch (cap) {
      case "round":
        var heads = makeRoundCap(
          iSegments[iSegments.length - 1],
          oSegments[0],
          offset,
        );
        var tails = makeRoundCap(
